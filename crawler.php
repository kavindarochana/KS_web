<?php
$start = "http://localhost:8080/KS_web/test.html";
//$start = "http://localhost:8080/wordpress/readme.html";
//$start = "http://localhost:8080/KIRP";


$pdo = new PDO('mysql:host=127.0.0.1;dbname=KS_web','root','');
...
function follow_links($url) {
	...
	foreach ($linklist as $link) {
		...
		if (!in_array($l, $already_crawled)) {
			 ...
			 $rows = $pdo->query("SELECT * FROM `index`
			 	WHERE url_hash='".md5($details->URL)."'");
			...
			// echo $rows."\n";
			 ...
			 if($rows > 0) {
			 	...
			 	$result = $pdo->prepare("UPDATE `index` SET :title, :description, :keywords, url:url, url:url_hash WHERE url_hash=:url_hash");
